...
**Contract 51 — Posts Delete (DELETE /posts/{id})** ([C51-REPORT](docs/reports/CONTRACT-51-REPORT.md)): handler `Delete` (hard-delete) en `internal/posts/http.go` + store `Posts.Delete` en `posts.go` con `post.validate` action:"delete", auth `AuthRequired` heredado de C47, logging slog + Metrics. `DELETE /posts/{id}` → 204/404/400. `http_test.go` +3 tests (re-sellado SHA `bdf8182f…`). Completa el CRUD HTTP REST. Habilitado por C48 (mattn CGO race-safe).
