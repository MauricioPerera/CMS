...
**Contract 36 — Posts CRUD** ([C36-REPORT](docs/reports/CONTRACT-36-REPORT.md)): store `internal/posts/posts.go` con Create/Update/Publish/Get/GetBySlug sobre tabla `posts` (C1), integrando el hook `post.validate` (C2) antes de cada escritura — rechazo aborta sin insertar. Oráculo 7/7 en `internal/posts/posts_test.go` (SHA256 `443abba5…`), data model OKF `knowledge/data_models/posts_crud.md` (enlazado desde `index.md`), spec `specs/CONTRACT-36-posts-crud.md`. Fix: `internal/db/init.go` registra el driver `file` de golang-migrate (import `_ "github.com/golang-migrate/migrate/v4/source/file"`).
