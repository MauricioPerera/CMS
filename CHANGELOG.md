...
**Contract 36 — Posts CRUD** ([C36-REPORT](docs/reports/CONTRACT-36-REPORT.md)): store `internal/posts/posts.go` con Create/Update/Publish/Get/GetBySlug sobre tabla `posts` (C1), integrando el hook `post.validate` (C2) antes de cada escritura — rechazo aborta sin insertar. Oráculo 7/7 en `internal/posts/posts_test.go` (SHA256 `6d72f54a…`, re-sellado por C37), data model OKF `knowledge/data_models/posts_crud.md` (enlazado desde `index.md`), spec `specs/CONTRACT-36-posts-crud.md`. Fix: `internal/db/init.go` registra el driver `file` de golang-migrate (import `_ "github.com/golang-migrate/migrate/v4/source/file"`).
...
**Contract 42 — Race tests read-path (verdicto)** ([C42-REPORT](docs/reports/CONTRACT-42-REPORT.md)): C42 (veredicto abierto sobre Windows). **VERDE tras C48**: `go test -race ./internal/...` = 0 data races, 0 checkptr crashes (mattn/go-sqlite3 CGO). CI linux ya no requerido para `-race` posts.
...
**Contract 47 — Posts Write API (Create/Update/Publish + auth)** ([C47-REPORT](docs/reports/CONTRACT-47-REPORT.md)): handlers write en `internal/posts/http.go` (POST /posts, PUT /posts/{id}, POST /posts/{id}/publish) con auth middleware inyectable (`WithAuth`/`AuthRequiredEnable`/`AuthRequired`) reutilizando hook `post.validate` (C2). 37/37 posts tests.
**Contract 48 — SQLite driver migration (modernc → mattn) para -race en Windows** ([C48-REPORT](docs/reports/CONTRACT-48-REPORT.md)): `modernc.org/sqlite` → `mattn/go-sqlite3 v1.14.50` (CGO puro C, gcc 15.2). `internal/db/init.go` (`database/sqlite`→`database/sqlite3`) + re-sellado oráculos `migrations_test.go`/`posts_test.go`. **`go test -race ./internal/...` → 0 races, 0 checkptr crashes.** Cierra C42 (INCONCLUSIVO → VERDE).
**Contract 49 — Fail-fast logger injection en NewHandler** ([C49-REPORT](docs/reports/CONTRACT-49-REPORT.md)): `NewHandler` panickea en `WithLogger(nil)` (fail-fast, no rely en `slog.Default` en prod). Cierra residual `slog.Default` (C44 #1); findings.json → 0. Ciclo observabilidad C41→C49 **completo**.
**Contract 50 — t.Parallel() concurrency guards en tests hooks + posts** ([C50-REPORT](docs/reports/CONTRACT-50-REPORT.md)): agrega `concurrent_runtime_test.go` (hooks) + `concurrent_http_test.go` (posts) con `t.Parallel()` + goroutines reales. Oráculos congelados C2/C36 preservados (SHA `5f14c71b…`/`f500f75f…`). `go test -race` 2/2 packages OK, 0 races, 0 flakes (3 runs).
**Contract 51 — Posts Delete (DELETE /posts/{id})** ([C51-REPORT](docs/reports/CONTRACT-51-REPORT.md)): handler `Delete` en `internal/posts/http.go` + store `Posts.Delete` en `posts.go` con `post.validate` action:"delete", auth `AuthRequired` heredado de C47, logging slog + Metrics. `DELETE /posts/{id}` → 204/404/400. `http_test.go` +3 tests. Completa el CRUD HTTP REST.
**Contract 54 — Soft-delete (deleted_at migration + Restore)** ([C54-REPORT](docs/reports/CONTRACT-54-REPORT.md)): migración `003_soft_delete` (ADD COLUMN deleted_at + idx), `Posts.Delete` pasa a **soft-delete** (UPDATE deleted_at), nuevo `Posts.Restore` + handler `POST /posts/{id}/restore`. Read path (`queryByID`/`getBySlug`/`List`) filtra `deleted_at IS NULL`. `TestMigrations_RollbackDropsTables` PASS (down: drop index antes de drop column). `http_test.go` +3 tests (`Delete_ThenRestore`, `Restore_NotFound`, `Restore_NotDeleted`). `migrations_test.go` oráculo C1 preservado (SHA `f1bf4aac…`). Re-sellado `posts-http-api.md` SHA `32a5c428…`.
**Contract 52 — Delete auth-reject + bad-ID paths (cobertura write path)** ([C52-REPORT](docs/reports/CONTRACT-52-REPORT.md)): +2 tests (`Delete_AuthRejected` → 401, `Delete_BadID` → 400) cerrando la cobertura de auth en el write path Delete. `http_test.go` re-sellado → SHA `fb49259c…`. Auth `AuthRequired` verificado bloqueando Delete antes del handler. Habilitado por C48 (mattn CGO race-safe).
**Contract 53 — PATCH /posts/{id} (update parcial)** ([C53-REPORT](docs/reports/CONTRACT-53-REPORT.md)): handler `Patch` en `internal/posts/http.go` + store `Posts.Patch` en `posts.go` (SET dinámico, sólo campos provistos via JSON raw map) con `post.validate` action:"patch", auth `AuthRequired` heredado. `PATCH /posts/{id}` → 200 (parcial, preserva campos no enviados) / 400 (no fields / bad json / bad id). Completa el CRUD HTTP REST. Habilitado por C48 (mattn CGO race-safe).
**Contract 55 — Rate limiting en write endpoints** ([C55-REPORT](docs/reports/CONTRACT-55-REPORT.md)): `TokenBucketRateLimiter` + `WithRateLimiter` option, aplicado en el middleware `AuthRequired` (sólo write methods: POST/PUT/PATCH/DELETE). `internal/posts/ratelimit.go` (Allow, rateLimitKey, isWriteMethod). 429 + `Retry-After` sobre excess. `http_test.go` +3 tests. Re-sellado `posts-http-api.md` SHA `854cbb82…` + `db-migrations.md` touch_only con `003_soft_delete.{up,down}.sql`.
**Contract 56 — E2E integration (httptest.NewServer sobre el mux real)** ([C56-REPORT](docs/reports/CONTRACT-56-REPORT.md)): 6 tests E2E en `internal/posts/e2e_test.go` levantando `httptest.NewServer(h)` sobre el mux Go 1.22 real → validan routing + `AuthRequired` (401/200) + rate-limit C55 (429) + soft-delete read filter C54 (GET borrado → 404) + Restore → 200. `go vet` clean. Re-sellado `posts-http-api.md` touch_only con `ratelimit.go`/`e2e_test.go`.
**Contract 64 — Deploy gopress como Cloudflare Worker (cmd/server + wrangler)** ([C64-REPORT](docs/reports/CONTRACT-64-REPORT.md)): entry point nuevo `cmd/server/main.go` (SQLite + migrations C1 + `posts.Handler` en `:PORT`, env CMS_*). Smoke test local PASS (healthz=200, posts=200). Cross-compile Linux en CI (host Windows+MinGW falla headers). `wrangler.jsonc` + `workers/wrangler/src/worker.js` (proxy HTTP al binary via @cloudflare/sandbox). `Makefile` build/smoke/deploy. `go build/vet` + `-race` 0. Oráculos preservados.
**Contract 63 — Dependency bump (apply C62 remediation)** ([C63-REPORT](docs/reports/CONTRACT-63-REPORT.md)): go.uber.org/atomic v1.7.0 -> v1.11.0 (indirect, compatible Go 1.22, build+vet+race OK); golang-migrate v4.15.2 -> v4.19.1 RECHAZADO (v4.19.1 exige go directive 1.24, rompe project constraint Go 1.22). dependency-eol/scan/findings.json actualizado (7 findings). validate_dependency_eol_findings PASS.
**Contract 62 — Dependency EOL / supply-chain scan (kdd-dependency-eol-scan)** ([C62-REPORT](docs/reports/CONTRACT-62-REPORT.md)): 6 deps en `go.mod` (0 EOL/unmaintained, 4 current-latest low, 2 behind medium). `dependency-eol/scan/findings.json` real (6 findings). `validate_dependency_eol_findings` PASS. CVE-scan fuera de scope (dominio separado `go-licenses`/govulncheck).
**Contract 61 — Privacy/PII scan (kdd-privacy-scan)** ([C61-REPORT](docs/reports/CONTRACT-61-REPORT.md)): 3 findings PII latente en schema (`users.email`/`sessions`/`posts.author_id`, base legal none-declared) vs runtime HTTP que no recolecta PII directamente. `privacy/scan/findings.json` real (3 findings). `validate_privacy_findings` PASS.
**Contract 58 — Compliance / license scan (kdd-compliance-scan)** ([C58-REPORT](docs/reports/CONTRACT-58-REPORT.md)): 6 deps (4 MIT + 2 MPL-2.0 weak-copyleft compatible MIT), 0 riesgo. `compliance/scan/findings.json`. `validate_compliance_findings` PASS (6 findings).
**Contract 60 — Security hardening (remediar findings C59)** ([C60-REPORT](docs/reports/CONTRACT-60-REPORT.md)): `WithBodyLimit(n)` → `MaxBytesReader` + Content-Length check en `AuthRequired` (writes-only, 413) para el finding MEDIUM `gopress/posts-create-no-body-size-limit`; fail-fast `AuthRequiredEnable(true)` + `auth=nil` → 500 (no fail-open silente) para el finding HIGH `goprin/posts-auth-default-disabled`. Default preservado (`authRequired=false`, `bodyLimit=0`) — BC oráculos. 4 tests C60 nuevos en `internal/posts/security_hardening_test.go` (SHA `3a227744…`). `validate_contracts` + `preflight` 19/19 + 744/744 + `-race` 0.
**Contract 59 — Security scan (kdd-security-scan)** ([C59-REPORT](docs/reports/CONTRACT-59-REPORT.md)): code-level scan de `internal/` + `db/` (2 findings: HIGH auth fail-open default `AuthRequired` + MEDIUM unbounded body DoS en Create/Patch/Update/Delete/Restore). `security/scan/{scan-manifest,findings,coverage}.json` sellados por `finalize_scan_contract.py` + `report.md` proyectado. `validate_security_findings.py` PASS (2 findings). Supply-chain (dependencias) fuera de scope (dominio política separado).
**Coherencia post-C48/C55/C56 — Refresco race analysis:** actualizado `knowledge/data_models/posts_race_analysis.md` (description header: "blocked por modernc → race-safe Windows tras C48") + `docs/reports/CONTRACT-42-REPORT.md` (verdicto INCONCLUSIVO → CERRADO post-C48; recomendación operativa refrescada: `-race` Windows VERDE tras mattn). Consistencia C48→C57 restaurada.
