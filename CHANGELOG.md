...
**Contract 52 — Delete auth-reject + bad-ID paths (cobertura write path)** ([C52-REPORT](docs/reports/CONTRACT-52-REPORT.md)): +2 tests (`Delete_AuthRejected` → 401, `Delete_BadID` → 400) cerrando la cobertura de auth en el write path Delete. `http_test.go` re-sellado → SHA `fb49259c…`. Auth `AuthRequired` verificado bloqueando antes del handler. Habilitado por C48 (mattn CGO race-safe).
